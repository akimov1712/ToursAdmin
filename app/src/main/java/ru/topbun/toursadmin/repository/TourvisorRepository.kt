...
import ru.topbun.toursadmin.models.TypeContent.departure
...
import ru.topbun.toursadmin.models.city.CityResponse
...
class TourvisorRepository(private val api: TourvisorApi) {
    ...
    suspend fun getStars() = api.getContent(stars).body<StarsResponse>().lists.stars.star
    suspend fun getMeal() = api.getContent(meal).body<MealResponse>().lists.meals.meal
    suspend fun getDeparture() = api.getContent(departure).body<CityResponse>().lists.departures.departure
